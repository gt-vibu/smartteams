...
interface ScreenPayrollViewProps {
  persona: Persona;
  activeTab: string;
  setActiveTab: (value: string) => void;
  compensation: PayrollCompensation;
  payslips: PayslipSummary[];
  advanceList: AdvanceItem[];
  dynamicCalc: PayrollCalculation;
  handleDownloadStructureStatement: () => void;
  handleOpenPayslip: (slip: PayslipSummary) => void;
  isAdvanceModalOpen: boolean;
  setIsAdvanceModalOpen: (open: boolean) => void;
  advanceAmount: string;
  setAdvanceAmount: Dispatch<SetStateAction<string>>;
  advanceReason: string;
  setAdvanceReason: Dispatch<SetStateAction<string>>;
  handleRequestAdvance: FormEventHandler<HTMLFormElement>;
  selectedPayslipModal: PayslipData | null;
  setSelectedPayslipModal: Dispatch<SetStateAction<PayslipData | null>>;
  selectedStructureModal: SalaryStructureDocumentData | null;
  setSelectedStructureModal: Dispatch<SetStateAction<SalaryStructureDocumentData | null>>;
}

export function ScreenPayrollView({
  persona,
  activeTab,
  setActiveTab,
  compensation,
  ...
  advanceList,
  dynamicCalc,
  handleDownloadStructureStatement,
  handleOpenPayslip,
  isAdvanceModalOpen,
...
}: ScreenPayrollViewProps) {
  return (
    <div className="space-y-6">
      {/* ── ZOHO-GRADE HERO SUMMARY BANNER ── */}
      <div className="bg-white dark:bg-card rounded-xl border border-slate-200/90 dark:border-slate-800 p-5 shadow-xs">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5">
          {/* Employee Identity */}
          <div className="flex items-start gap-3.5">
            <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-primary to-indigo-600 text-white font-bold text-lg flex items-center justify-center shadow-xs shrink-0">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
            <div className="bg-slate-50 dark:bg-card p-3 rounded-lg border border-slate-200/80 dark:border-slate-800 text-left">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-3">
          <TabsList className="bg-slate-100 dark:bg-card p-1 rounded-lg">
        ...
        <TabsContent value="structure" className="space-y-4 mt-4">
          ...
          <div className="bg-slate-50 dark:bg-card p-4 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="h-9 w-9 rounded-lg bg-sky-50 dark:bg-sky-950/60 border border-sky-200 dark:border-sky-800/70 text-primary dark:text-sky-400 flex items-center justify-center shrink-0">
                <svg
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-slate-700 dark:text-slate-300 bg-white dark:bg-card px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800">
          ...
          <div className="bg-white dark:bg-card rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xs">
            <div className="p-4 bg-slate-50/80 dark:bg-card border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              <div>
                <span className="text-xs font-bold text-slate-900 dark:text-white">
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-100/75 dark:bg-card">
              ...
              <TableBody>
                ...
                <TableRow className="bg-slate-50/50 dark:bg-card/50 font-bold">
                  <TableCell
                    colSpan={5}
                ...
                {compensation.earnings.map((e, idx) => (
                ...
                {/* Subtotal Gross */}
                <TableRow className="bg-sky-50/60 dark:bg-card/60 font-bold border-t border-sky-200 dark:border-sky-800/50">
                ...
                {/* Total Annual CTC */}
                <TableRow className="bg-slate-100 dark:bg-card font-bold border-t border-slate-300 dark:border-slate-700">
        ...
        <TabsContent value="payslips" className="space-y-4 mt-4">
          <Card>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50 dark:bg-card">
                ...
                {payslips.map((slip) => (
                  ...
                  >
                    <TableCell className="px-4 font-bold text-slate-900 dark:text-white">
                      <div className="flex items-center gap-2">
                        <div className="h-7 w-7 rounded bg-sky-50 dark:bg-sky-950/50 text-primary dark:text-sky-300 font-bold text-xs flex items-center justify-center">
                          📅
                        </div>
                        <div>
                          <div>{slip.month}</div>
